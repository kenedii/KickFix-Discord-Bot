...
async def download_clip(url):
    ...
    clip_filepath = await download_and_assemble(clip_links, output_dir, clip_id)
...
async def download_and_assemble(urls, output_dir, output_filename):
    ...
    compression_command = [
        'ffmpeg', '-i', output_mp4, '-vf', 'scale=-1:720', '-c:v', 'h264_nvenc', '-crf', '23', '-preset', 'medium', compressed_mp4
    ]
